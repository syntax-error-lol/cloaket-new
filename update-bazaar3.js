  ...
  return (
    <Layout title="Bazaar" fixedHeight>
      <div className="flex flex-col h-full max-w-6xl mx-auto overflow-hidden gap-4 pb-4">
        ...
        {/* TOP ROW */}
        <div className="flex flex-wrap lg:flex-nowrap items-stretch gap-4 shrink-0">
          
          {/* Left Column: Title, Tokens */}
          <div className="flex flex-col gap-2 w-full sm:w-48 shrink-0">
            ...
            <div className="flex flex-col bg-card border border-card-border p-2.5 rounded-lg shadow-sm mt-1 flex-1 justify-center">
          ...
          <div className="flex-1 bg-card border border-card-border rounded-xl shadow-md p-3 flex flex-col justify-center min-w-0 md:min-w-[300px]">
          ...
          <div className="w-full sm:w-48 shrink-0 bg-card border border-card-border rounded-xl shadow-md p-3 flex flex-col justify-center gap-2 h-auto min-h-[96px]">
            ...
            {/* The visibility of clear button must not change height. We use opacity/pointer-events instead of unmounting. */}
            <Button 
              variant="outline" 
              size="sm" 
              onClick={() => { setSelectedPackName(null); setSelectedBlookName(null); setSearch(""); }}
              className={\`font-bold w-full h-9 text-xs transition-opacity \${(!selectedPackName && !selectedBlookName && !search) ? 'opacity-50 pointer-events-none' : 'opacity-100'}\`}
                ...
                {filteredListings.map(listing => (
                  ...
                  >
                    <div className="h-28 bg-secondary/40 relative flex items-center justify-center p-3">
                      ...
                      {listing.isMine && (
                        <div className="absolute top-1.5 left-1.5 px-1.5 py-0.5 rounded text-[9px] font-black uppercase bg-blue-500/80 text-white backdrop-blur z-10 shadow-[0_0_5px_rgba(59,130,246,0.5)]">
                          Yours
                        </div>
                      )}
                      
                      {/* Hover Overlay */}
                      <div className={\`absolute inset-0 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-[2px] z-20 \${listing.isMine ? 'bg-destructive/80' : 'bg-primary/80'}\`}>
                        <span className="font-black text-white text-md drop-shadow-md">
                          {listing.isMine ? 'Cancel' : 'Buy'}
                        </span>
                      </div>
                    </div>
                    <CardContent className="p-2.5 flex flex-col gap-1.5 relative z-10 bg-card">
