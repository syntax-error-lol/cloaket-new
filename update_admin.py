...
with open("artifacts/blacket-game/src/pages/admin.tsx", "r") as f:
    content = f.read()

# 1. Update imports
import_lucide = re.search(r'import \{ Loader2, ShieldAlert, Award, PackageOpen, Search, Sparkles, Check, X \} from "lucide-react";', content)
if import_lucide:
    content = content.replace(
        import_lucide.group(0),
...
# 2. Add mutations
mutations_str = """  const lookupMutation = useAdminLookup();
...
# 5. Add UI sections
name_effect_section = """                    </Button>
                  </div>
                </div>"""

new_sections = """                {/* Gift All Blooks Section */}
...
# Find the last occurrence of name_effect_section to append
parts = content.rsplit(name_effect_section, 1)
if len(parts) == 2:
    content = parts[0] + name_effect_section + "\n\n" + new_sections + parts[1]

# 6. Add dialog before </main>
dialog_html = """      {/* Delete Confirmation Dialog */}
...
with open("artifacts/blacket-game/src/pages/admin.tsx", "w") as f:
    f.write(content)
